...
## 4. Метрики оценки
### Таблица метрик
| Название метрики                                                                 | Описание                                                                 | Характеристика                                                                 |
|-----------------------------------------------------------------------------------|-------------------------------------------------------------------------|--------------------------------------------------------------------------------|
| **mean_hits_per_sample_percent**<br>(MEAN_H)                                      | Доля попаданий истинного источника в FoV                                | Средний процент "приблизительно верных" предсказаний в сессии                  |
| **median_hits_per_sample_percent**<br>(MEDIAN_H)                                  | Доля попаданий истинного источника в FoV                                | Медианный процент "приблизительно верных" предсказаний                         |
| **mean_accuracy_hits_per_sample_percent**<br>(MEAN_AH)                            | Доля хитов с шириной FoV ≤ заданной точности                            | Средний процент "метких" предсказаний                                          |
| **median_accuracy_hits_per_sample_percent**<br>(MEDIAN_AH)                        | Доля хитов с шириной FoV ≤ заданной точности                            | Медианный процент "метких" предсказаний                                        |
| **mean_abs_direction_difference_per_sample_percent**<br>(MEAN_ADD)                | Средняя абсолютная разница между истинным и предсказанным направлением | Отклонение направления в % от 360° (чем меньше, тем точнее)                    |
| **median_abs_direction_difference_per_sample_percent**<br>(MEDIAN_ADD)            | Медианная абсолютная разница направлений                                | Устойчивая оценка отклонения направления                                       |
| **mean_width_by_<sample_size_range>**<br>(MEAN_W_<A_B>)                           | Средняя ширина FoV для диапазона измерений (напр., 0-10 точек)          | Зависимость ширины FoV от количества данных                                    |
| **median_width_by_<sample_size_range>**<br>(MEDIAN_W_<A_B>)                       | Медианная ширина FoV для диапазона измерений                            | Устойчивая оценка ширины FoV                                                   |

**Примечания:**
- Формат для диапазонов: `<sample_size_range>` заменяется на `A_B` (например, `mean_width_by_0_10` → MEAN_W_0_10).
- Все метрики рассчитываются для одной сессии (набора измерений).
