...
## 4. Метрики оценки
### Таблица метрик
| Полное название метрики                                | Сокращение | Описание                                                                 |
|--------------------------------------------------------|------------|-------------------------------------------------------------------------|
| **mean_hits_per_sample_percent**                       | MEAN_H     | Средний процент попаданий в FoV в рамках сессии                        |
| **median_hits_per_sample_percent**                     | MEDIAN_H   | Медианный процент попаданий в FoV                                      |
| **mean_accuracy_hits_per_sample_percent**              | MEAN_AH    | Средний процент "метких" предсказаний (ширина FoV ≤ порога)            |
| **median_accuracy_hits_per_sample_percent**            | MEDIAN_AH  | Медианный процент "метких" предсказаний                                |
| **mean_abs_direction_difference_per_sample_percent**   | MEAN_ADD   | Средняя абсолютная разница направлений (в % от 360°)                   |
| **median_abs_direction_difference_per_sample_percent** | MEDIAN_ADD | Медианная абсолютная разница направлений                               |
| **mean_width_by_<sample_size_range>**                  | MEAN_W     | Средняя ширина FoV для диапазона измерений (напр., `MEAN_W_0_10`)      |
| **median_width_by_<sample_size_range>**                | MEDIAN_W   | Медианная ширина FoV для диапазона измерений (напр., `MEDIAN_W_10_20`) |

**Примечания:**
- `<sample_size_range>` заменяется на диапазон количества измерений (например, `0_10` для 0-10 вершин).
- Все метрики рассчитываются на уровне сессии (набора измерений).
- **"Меткое предсказание"** — хит с шириной FoV ≤ заданного порога точности.
