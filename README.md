...
Датасет хранится в файле dataset.csv следующей структуры:
        ...
        source_id,longitude,latitude,rssi,source_longitude,source_latitude,azimuth,distance
        0,1185,567,-128.67,638,587,272.01,547.93
        0,1208,638,-130.44,638,587,264.87,573.13
        ...
        752,223,624,-115.12,507,608,93.38,283.56
        752,172,646,-118.5,507,608,96.55,336.57
...
### Выбор типа механизма Message passing

На фоне прочих методов выделяется Graph Attention Networks (GAT), который улучшает механизм передачи сообщений путем введения механизма внимания, который позволяет узлам по-разному взвешивать сообщения от различных соседей.
![image](https://github.com/user-attachments/assets/cfebba88-7732-4d1f-ba93-441a75e2506a)

где a - функция внимания, обычно реализуемая через нейронную сеть, а α_{ij} - нормализованные веса внимания.

Преимущества GAT:

Способность динамически присваивать важность различным соседям

Хорошая работа в гетерогенных графах

Повышенная выразительность модели

Недостатки GAT:

Повышенная вычислительная сложность

Большее количество параметров для обучения

Таким образом схема сети будет выглядеть следующим образом:

```mermaid
graph TD
    A["Input Features<br>(RSSI + координаты)"] --> B["Графовый слой 1 (GAT)"]
    B --> C["Графовый слой 2 (GAT)"]
    C --> D["Графовый слой 3 (GAT)"]
...
Для азимута (предсказываются синус и косинус)
Среднеквадратичная ошибка (MSE) между предсказанными и истинными значениями синуса/косинуса:

$$
Loss_{\text{azimuth}} = \frac{1}{N} \sum_{i=1}^N \left[ 
  \left( \sin\theta_{\text{pred}}^{(i)} - \sin\theta_{\text{true}}^{(i)} \right)^2 + 
  \left( \cos\theta_{\text{pred}}^{(i)} - \cos\theta_{\text{true}}^{(i)} \right)^2 
\right]
$$

Для расстояния
Стандартная MSE между предсказанными и истинными расстояниями:

$$
Loss_{\text{distance}} = \frac{1}{N} \sum_{i=1}^N \left( d_{\text{pred}}^{(i)} - d_{\text{true}}^{(i)} \right)^2
$$

Итоговая функция потерь
Комбинированная потеря с весовыми коэффициентами:

$$
Loss = \alpha \cdot Loss_{\text{azimuth}} + \beta \cdot Loss_{\text{distance}}
$$
