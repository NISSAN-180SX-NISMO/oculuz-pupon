...
|-----------------------------------------------------------------------------------|-------------------------------------------------------------------------|--------------------------------------------------------------------------------|
| **mean_hits_percent**<br>(MEAN_H)                                      | Доля попаданий истинного источника в FoV                                | Средний процент "приблизительно верных" предсказаний в сессии. Чем больше тем лучше          |
| **median_hits_percent**<br>(MEDIAN_H)                                  | Доля попаданий истинного источника в FoV                                | Медианный процент "приблизительно верных" предсказаний. Чем больше тем лучше                 |
| **mean_accuracy_hits_percent**<br>(MEAN_AcH)                            | Доля хитов с шириной FoV ≤ заданной точности                            | Средний процент "метких" предсказаний. Чем больше тем лучше                                  |
| **median_accuracy_hits_percent**<br>(MEDIAN_AcH)                        | Доля хитов с шириной FoV ≤ заданной точности                            | Медианный процент "метких" предсказаний. Чем больше тем лучше                                |
| **mean_abs_direction_difference_percent**<br>(MEAN_ADD)                | Средняя абсолютная разница между истинным и предсказанным направлением  | Среднее отклонение направления. Чем меньше тем лучше                                         |
| **median_abs_direction_difference_percent**<br>(MEDIAN_ADD)            | Медианная абсолютная разница между истинным и предсказанным направлением| Медианное отклонение направления. Чем меньше тем лучше                                       |
| **mean_abs_width_by_<sample_size_range>**<br>(MEAN_AW_<A_B>)                | Средняя ширина FoV для диапазона измерений                              | Зависимость ширины FoV от количества данных. Хорошо когда чем меньше вершин тем больше ширина|
| **median_abs_width_by_<sample_size_range>**<br>(MEDIAN_AW_<A_B>)            | Медианная ширина FoV для диапазона измерений                            | Зависимость ширины FoV от количества данных. Хорошо когда чем меньше вершин тем больше ширина|

**Примечания:**
- Всякий предмет метрики (все что после mean/median) рассчитывается для одного семпла (доля берется в рамках одного семпла, а значение ширины/отклонения усредняется ) затем эти значения используется для рассчета mean/median по всей выборке. 
